...
describe("Short URL Generator", () => {
  ...
  describe("image expiry", () => {
    const expStore = new Map<string, { body: ArrayBuffer; contentType?: string; expiresAt?: string }>();
    const mockEnvExp = {
      IMAGE_R2: {
        put: mock(async (key: string, value: ArrayBuffer, opts: any) => {
          expStore.set(key, {
  ...
  describe("one-time images (burn after reading)", () => {
    const imgStore = new Map<string, { body: ArrayBuffer; contentType?: string; expiresAt?: string; oneTime?: string }>();
